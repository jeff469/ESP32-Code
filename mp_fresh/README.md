...
- Wind: fetched from ECCC GeoMet SWOB API for station code in `config.WIND_STATION_CODE` (default `COGI`).
...
Wi-Fi/API: fill in `WIFI_SSID`/`WIFI_PASS` in `config.py` so wind speed/direction can be fetched for binning. A 5-minute cache (`WIND_API_CACHE_SECONDS`) avoids hammering the GeoMet API.
